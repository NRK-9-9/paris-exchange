...
const ExchangeTableRow = ({
  ...
  return (
    <tr
      className={`${color} hover:bg-sky-600 hover:text-secondary-content text-neutral-500`}
    >
      <td className="p-2">
        <FlagCoded code={code} />
          ...
          <div>
            {type === "sell" ? (
              <div>
                {reserveType === "1" ? (
                  <YesNoQuestion
                    Question1={`${buyCur} ${currency} = `}
                    Question2={`${buyEur} EUR?`}
                  ...
                  <YesNoQuestion
                    Question1={`${buyEur} EUR = `}
                    Question2={`${buyCur} ${currency}?`}
                    noFunc={() => setquestionCUR(null)}
                    yesData={{
                      orderType: "buy",
                      currency: currency,
              ...
              <div>
                {reserveType === "1" ? (
                  <YesNoQuestion
                    Question1={`${buyEur} EUR = ${buyCur} ${currency}`}
                    Question2={``}
                    noFunc={() => setquestionCUR(null)}
                    yesData={{
                      orderType: "sell",
